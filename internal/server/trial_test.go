...
func TestTrialMiddleware_writesBlocked(t *testing.T) {
	...
	cases := []struct {
		...
		{"PUT", "/api/extras/announcements/123"},
...
func TestTrialMiddleware_proTierBypassesGate(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"title": "Alice"})
	req := httptest.NewRequest("POST", "/api/announcements", bytes.NewReader(body))
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	if w.Code == http.StatusPaymentRequired {
...
// newTestDB is a local helper since announcements has no existing
// test file that defines one.
func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "announcements-trial-test-*")
	if err != nil {
