package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-announcements/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-announcements"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/announcements",s.handleList)
    s.mux.HandleFunc("POST /api/announcements",s.handleCreate)
    s.mux.HandleFunc("POST /api/announcements/{id}/read",s.handleRead)
    s.mux.HandleFunc("DELETE /api/announcements/{id}",s.handleDelete)
