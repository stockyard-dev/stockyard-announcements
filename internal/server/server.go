...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("announcements: license persist failed: %v", err)
	...
	s.limits = ProLimits()
	s.limMu.Unlock()
	log.Printf("announcements: license activated via dashboard, persisted to %s/%s", s.dataDir, licenseFilename)
...
func (s *Server) tierInfo(w http.ResponseWriter, r *http.Request) {
	...
	if tier == "trial-required" {
		...
		resp["message"] = "Your trial is not active. Reads work, but you cannot create or change announcements until you start a 14-day trial or activate an existing license key."
	} else {
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("announcements: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("announcements: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"announcements": oe(s.db.Search(q, filters))})
		return
	}
